...
int main(int argc, char *argv[])
{
    ...
    printf("La menor temperatura mínima histórica registrada en la ciudad de Córdoba es %d \n", min_temp(array));

    int maxtemp[YEARS];
    procedimiento(array, maxtemp);
    for (unsigned int i = 0; i < YEARS; i++)
    {
        printf("La mayor temperatura máxima registrada durante %d fue %d. \n", i + FST_YEAR, maxtemp[i]);
    }

    month_t maxpremens[YEARS];
    mes_max_pre(array, maxpremens);
    for (unsigned int i = 0; i < YEARS; i++)
    {
        printf("El mes de mas precipitaciones de %d fue el mes %d. \n", i + FST_YEAR, maxpremens[i]);
    }
